...
class GeyserAccessory extends BaseAccessory {
    constructor(platform, homebridgeAccessory, deviceConfig) {
        ...
        super(
            ...
            homebridgeAccessory,
            deviceConfig,
            Accessory.Categories.GEYSERWISE_MAX1,
            Service.thermostat
        );
            ...
            if (statusMap.code === 'temp_set' || statusMap.code === 'temp_set_f') {
                ...
                if (!this.temp_set_range) {
                    if (statusMap.code === 'temp_set') {
                        this.temp_set_range = { 'min': 30, 'max': 75 }
                    } else {
                        this.temp_set_range = { 'min': 31, 'max': 74 }
                    }
                }
    ...
    getTempSetDPRange() {
        ...
        for (const funcDic of this.functionArr) {
            ...
            switch (funcDic.code) {
                case 'temp_set':
                    tempSetRange = isnull ? { 'min': 30, 'max': 75 } : { 'min': parseInt(valueRange.min), 'max': parseInt(valueRange.max) }
                    break;
                case 'temp_set_f':
                    tempSetRange = isnull ? { 'min': 31, 'max': 74 } : { 'min': parseInt(valueRange.min), 'max': parseInt(valueRange.max) }
                    break;
                default:
                    break;
            }
...
module.exports = GeyserAccessory;