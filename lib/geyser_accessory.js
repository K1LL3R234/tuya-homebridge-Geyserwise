...
class HeaterAccessory extends BaseAccessory {
      ...
      if (statusMap.code === 'temp_set' || statusMap.code === 'temp_set_f') {
        ...
        if (!this.temp_set_range) {
          if (statusMap.code === 'temp_set') {
            this.temp_set_range = { 'min': 45, 'max': 75 }
          } else {
            this.temp_set_range = { 'min': 45, 'max': 75 }
          }
        }
  ...
  getTempSetDPRange() {
    ...
    for (const funcDic of this.functionArr) {
      ...
      switch (funcDic.code) {
        case 'temp_set':
          tempSetRange = isnull ? { 'min': 45, 'max': 75 } : { 'min': parseInt(valueRange.min), 'max': parseInt(valueRange.max) }
          break;
        case 'temp_set_f':
          tempSetRange = isnull ? { 'min': 45, 'max': 75 } : { 'min': parseInt(valueRange.min), 'max': parseInt(valueRange.max) }
          break;
